 ...
 *
 * `/cron <cron>` posts a start notice (so a fresh session opens a turn and
 * shows up in the list), then fires one tick at every wall-clock minute
 * matching the cron expression, re-prompting the agent each time, until the
 * human stops it with `/cron stop`.
 *
...
const HELP = [
  "Runs the agent on a cron schedule: a start notice, then one tick at each",
...
class CronRuntime {
  ...
  /**
   * Post the one-shot start notice. This wakes the driver once so the session
   * opens a turn and becomes visible (DSH hides a blank session until a turn
   * starts); the objective itself still waits for the first wall-clock tick.
   * Deferred so the command's own lifecycle events settle first.
   */
  confirmStart() {
    setTimeout(() => {
      if (!this.running || this.stopped) return;
      try {
        this.agent.followup(this.confirmationMessage());
      } catch {
        // Agent already gone; the armed wall-clock timer is unaffected.
      }
    }, 0);
  }

  /** Build the one-shot confirmation notice for a freshly started schedule. */
  confirmationMessage() {
    const lines = [`Cron schedule started: ${JSON.stringify(this.cronExpr)}.`];
    if (this.objective) lines.push(`Objective: ${JSON.stringify(this.objective)}`);
    if (this.nextFireAt !== null) {
      lines.push(`First tick at: ${new Date(this.nextFireAt).toLocaleString()}.`);
    }
    lines.push(
      "Acknowledge this notice and wait — the objective runs at each scheduled " +
        "tick until the user runs /cron stop.",
    );
    return createUserMessage({
...
function executeCommand(ctx, invocation, state) {
  ...
  const runtime = state.startCron(agent, state.defaultCron, input, { confirm: true });
  return { kind: "success", text: runtime.startedText() };
}
...
function apply(ctx, config = {}) {
  ...
  const startCron = (agent, cronExpr, objective, options = {}) => {
    ...
    if (options.confirm) runtime.confirmStart();
    return runtime;
  };
  ...
  const resumeCron = (agent) => {
    ...
    const runtime = startCron(agent, saved.cronExpr, saved.objective ?? undefined, { confirm: true });
    return { kind: "success", text: runtime.startedText() };
  };
